...
&emsp;&emsp;最后，定义获取权重的函数`get.weights`以及计算邮件优先级的函数`rank.message`。将邮件进行优先级的排序并展示一些结果。
...
# Find splits again
train.paths <- priority.df$Path[1:(round(nrow(priority.df) / 2))] %>% as.vector()
...
final.df$Date <- date.converter(final.df$Date, pattern1, pattern2) %>% as.POSIXct()
     ...
     From = formatter("span",
                      style = x ~ ifelse(x == "ville.skytta@iki.fi", 
                                         ...
                                         ifelse(x == "rssfeeds@spamassassin.taint.org",
                                                ...
                                                NA))),
     Priority = formatter("span", style = x ~ style(color = ifelse(x == 1, "green", "red")),
