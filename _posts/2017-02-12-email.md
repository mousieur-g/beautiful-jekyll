...
spam.occurrence <- sapply(1:nrow(spam.matrix),
                         function(p) sum(spam.matrix[p, ] != 0) / spam.n)
...
```r
new_theme <- theme_update(panel.background = element_rect(fill = NA),
...
p <- head(spam.dt, 10) %>% ggplot(aes(term, occurrence, fill = term)) 
p +  geom_bar(stat = "identity", aes(reorder(term, occurrence)), show.legend = F) + 
