...
&emsp;&emsp;利用这样的等价理论，我们就能建立一个简单的垃圾邮件识别系统。定义一个名为`classify.email`的函数，计算上述公式中的联合密度*P(Y, X)*。
...
+ *path*: 待判邮件的存储路径
+ *training.dt*: 给定一个训练集的词频信息，可以是垃圾邮件的词频，也可以是正常邮件的
+ *prior*: 先验概率，相当于上述公式中的*P(Y)*
+ *c*: 对于没有出现在训练集中的词频，设定一个极小的概率值，这里默认为*0.001%*
+ *theta*: 由于`R`中计算的精度问题，如果直接计算概率值，结果往往非常小，在`R`中均会显示为0，因此设置这样一个参数，成倍地放大概率值，尽可能避免这样的情况发生。同时，这不影响之后概率值之间的比较
