...
&emsp;&emsp;利用这样的等价理论，我们就能建立一个简单的垃圾邮件识别系统。定义一个名为`classify.email`的函数，计算上述公式中的*P(Y, X)*。
...
classify.email <- function(path, training.dt, prior = 0.5, c = 1e-5, theta = 100)
                  {
                    ...
                    if(length(msg.match) < 1)
                      ...
                      {
                        match.probs <- training.dt[ , occurrence][match(msg.match, training.dt[ , term])] * theta
